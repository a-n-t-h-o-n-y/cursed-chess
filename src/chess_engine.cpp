...
bool Chess_engine::game_over() const {
    int count{0};
...
void Chess_engine::reset_game() {
    this->reset_board();
    move_count_ = 0;
    this->status_message_sig("ᴳᵃᵐᵉ ʳᵉˢᵉᵗ\n");
}

std::vector<Position> Chess_engine::get_moves(Position pos) {
    auto* piece = find_piece(pos);
    if (piece == nullptr) {
        return std::vector<Position>{};
    }
    piece->update_valid_moves([this](Position p) { return this->get_side(p); });
    auto moves = piece->valid_moves();
    auto new_end =
        std::remove_if(std::begin(moves), std::end(moves), [](const auto& pos) {
...
Chess_engine::get_all_potential_moves(Side side) {
...
bool Chess_engine::make_move(Position pos1, Position pos2) {
...
const std::vector<std::unique_ptr<Piece>>& Chess_engine::get_pieces() const {
    return pieces_;
}

// Private Members

void Chess_engine::flip_side() {
...
void Chess_engine::remove_piece(Position p) {
...
Side Chess_engine::get_side(Position p) const {
...
Piece* Chess_engine::find_piece(Position p) const {
    for (const auto& piece : pieces_) {
...
bool Chess_engine::check_move(Position pos1, Position pos2) {
...
std::string Chess_engine::int_to_tiny(const std::string& number) {
    std::string result;
...
void Chess_engine::report_move(Position p1, Position p2) {
    ...
    std::map<std::string, std::string> figs{{"♚", "ᴷ"}, {"♛", "੧"}, {"♜", "ᴿ"},
                                            {"♝", "ᴮ"}, {"♞", "ᴺ"}, {"♟", ""}};
...
void Chess_engine::reset_board() {
    ...
    pieces_.push_back(std::make_unique<Knight>(Side::White, Position{1, 2}));
    pieces_.push_back(std::make_unique<Bishop>(Side::White, Position{1, 3}));
    ...
    pieces_.push_back(std::make_unique<Bishop>(Side::White, Position{1, 6}));
    pieces_.push_back(std::make_unique<Knight>(Side::White, Position{1, 7}));
    ...
    pieces_.push_back(std::make_unique<Knight>(Side::Black, Position{8, 2}));
    pieces_.push_back(std::make_unique<Bishop>(Side::Black, Position{8, 3}));
    ...
    pieces_.push_back(std::make_unique<Bishop>(Side::Black, Position{8, 6}));
    pieces_.push_back(std::make_unique<Knight>(Side::Black, Position{8, 7}));
